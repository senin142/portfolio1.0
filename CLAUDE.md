...
9. **Row Level Security must stay enabled on every table in this Supabase project.** Found during a 2026-09-14 red-team pass: creating tables via raw Sequelize migrations (rather than Supabase's own UI) skips the usual RLS nudge, and Supabase's auto-generated PostgREST API defaults to exposing any table without RLS to anyone holding the public "publishable" key — this briefly exposed the full `users` table including password hashes. Fixed via `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`, with a `cms_app_full_access` policy per table granting only the app's own `cms_app` role (see #10) access — **any new migration that adds a table must also enable RLS and add that policy**, it will not happen by default.
10. **The app connects to Supabase as a scoped `cms_app` role, not the `postgres` superuser** (migration `20260101000010-scoped-app-role.js` in `backend/`) — least-privilege, only `SELECT/INSERT/UPDATE/DELETE` on this app's own tables. The `postgres` superuser credential is kept separately (`DB_SUPERUSER_*` in `.env`) only for re-running that migration if the role ever needs recreating.
11. **Auth was hardened (2026-09-14):** short-lived (15m) access tokens paired with rotating, revocable refresh tokens (`refresh_tokens` table, only a hash is ever stored) — `POST /auth/refresh` rotates and rejects reuse of an already-rotated token, `POST /auth/logout` revokes on demand. `frontend/src/lib/api.ts` retries once on a 401 via a shared in-flight refresh, so this is invisible during normal use. Also added an audit log (`audit_logs` table, `GET /audit-log` admin-only) covering auth events and destructive/privileged actions, and rate limiting (`@nestjs/throttler`, 5/min on `/auth/login` and `/auth/signup`).
